...
void NuSetShaderState(enum shadertypes_e type, enum nuvtxtype_e vtxtype) {
    ...
    if (type == -1) {
        ...
        SetVertexShader(0x59);
    }
    else {
        if (Shaders[type].vertshader) {
            SetVertexShader(Shaders[type].vertshader);
        } else {
            SetVertexShader(0);
        }
...
//NGC MATCH
struct nuvec_s* GetPlayerPosition(void) {
    if (player == NULL) {
...
void SetupShaders(struct nugeomitem_s* geomitem) {
    ...
    switch(shader) {
        ...
        case BRDFGOLD: // 10
            ...
            memcpy(&eyepos, &cammtx->_30, sizeof(struct nuvec_s));
